...
function Sankey() {
  ...
  function computeNodeLayers({nodes}) {
    ...
    let layerNos = {}, uniqueLayers = 0;
    nodes.forEach(d => {layerNos[d.m_layer] = 1});
    for (let layerNo in layerNos) {uniqueLayers ++;}
    const x = uniqueLayers;
    const kx = (x1 - x0 - dx) / (x - 1);
    const columns = new Array(x);
    for (const node of nodes) {
      const i = align.call(null, node, x);//Math.max(0, Math.min(x - 1, Math.floor(align.call(null, node, x))));
      node.layer = i;
      node.x0 = x0 + i * kx;
      node.x1 = node.x0 + dx;
